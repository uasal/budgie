...
from budgie.core import (
    Budget,
    find_vals,
)  # Plantuml_Writer. -- don't understand why imports are done like this.
...
class WaveFrontError(Budget):
    def __init__(self, budget_path):
        ...
        self.path_to_yaml = Path(budget_path)

        self.wfe = Budget(self.path_to_yaml)
    ...
    def run_report(self, output_dir):
        ...
        total_allocation_wfe = (
            f"- Total Allocated WFE: {total_allocation * 1e9:0.2f} [nm]\n"
        )
        ...
        report = (
            ...
            + total_spec_wfe
            + total_allocation_wfe
            + total_margin_spec_wfe
            + total_margin_cbe_wfe
            + end
            + report_budget
        )
        print(report)
        ...
        print(
            "Report Results Generated. Verify to the reports directory markdown file for the report results."
        )
